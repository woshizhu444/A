#!/usr/bin/env bash
# r21 – restore 0/1/2/3 menu, multi-billing, fix proj var & key-delete issues
...
check_env() {
  ...
  gcloud auth list --filter=status:ACTIVE --format='value(account)' | grep -q . \
    || { log ERROR "请先 gcloud auth login"; exit 1; }
}
...
enable_services() {
  ...
  local svc
  for svc in "$@"; do
    gcloud services list --enabled --project="$proj" --filter="$svc" \
...
gen_key() {
  local proj=$1 sa=${2:-}
  [[ -z $sa ]] && { log ERROR "gen_key 缺少 service-account 参数"; return 1; }
  ...
  if (( ${#local_keys[@]} )); then
    if ask_yes_no "[$proj] 本地已有密钥 (${#local_keys[@]}). 生成新密钥?" Y; then
      ...
      if ask_yes_no "[$proj] 删除云端旧密钥(保留最新)?" N; then
        ...
        for k in "${key_ids[@]}"; do
          [[ $k == "$latest" ]] && continue
          gcloud iam service-accounts keys delete "$k" \
                 --iam-account="$sa" --quiet \
          || log WARN "[$proj] 跳过无法删除的密钥 $k"
        done
      fi
    else
...
create_project() {
  ...
  provision_sa "$pid"
  PROJECTS+=("$pid")   # 写回数组供 show_status
}

process_projects() {                 # 并行启 API（限速）→ 顺序置备
  local proj running=0
  for proj in "$@"; do
    [[ -z $proj ]] && continue
    (
      sleep $((RANDOM%3))
  ...
  wait
  for proj in "$@"; do [[ -z $proj ]] && continue; provision_sa "$proj"; done
}

show_status() {
  printf "\n当前项目状态 (Billing: %s)\n" "$BILLING_ACCOUNT"
  local proj api keycount
...
main() {
  ...
  # 多账单账户：先问单一/批量
  local mode; mode=$(prompt_choice $'检测到多个结算账户，选择模式：\n 1) 单一账户\n 2) 批量全部' "1|2" "1")
  if [[ $mode == 2 ]]; then
